...
class Poll(models.Model):
	PL_CODE = models.CharField(max_length=200)                             # the code of the poll
	PL_TITLE = models.CharField(max_length=200)                            # the title of the poll
	PL_SHRBODY = models.CharField(max_length=300)                          # the short body of the poll
	PL_BODY = models.CharField(max_length=1000)                            # the body (main text) of the poll
	PL_CREATION = models.DateTimeField(default = timezone.now())           # the creation date and time of the poll
	PL_STARTDT = models.DateTimeField()                                    # the starting date and time of the poll
	PL_ENDDT = models.DateTimeField()                                      # the ending date and time of the poll
	PL_DURATION = models.IntegerField(default=0)                           # the duration of the poll
	PL_CODE2 = models.CharField(max_length=200, blank = True)              # the code2 of the poll (protocol number, approval number)
	USER = models.ForeignKey(User, on_delete=models.DO_NOTHING)            # the user (admin) who created the poll
	UGROUP = models.ForeignKey(Group, on_delete=models.DO_NOTHING)         # the group of the user that created the poll
	POLLCAT = models.ForeignKey(PollCat, on_delete=models.DO_NOTHING)      # the category of the poll
	def __str__(self): 
...
class PollChoice(models.Model):
	...
	PC_CHOICE = models.CharField(max_length=100)                                          # the vote choice
	PC_VOTES = models.IntegerField(default=0)                                             # the number of votes
	def __str__(self):
