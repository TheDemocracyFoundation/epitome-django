...
If you choose to install Epitome on Heroku, please click the button above, the fields of the required variables should be already populated (such as the the required config variable "EPITOME_SECRET_KEY", which should contain a 50-character string including numbers and symbols. If you are creating the key manually, you can use [an online tool to help you with the key generation](https://www.miniwebtool.com/django-secret-key-generator/)).
